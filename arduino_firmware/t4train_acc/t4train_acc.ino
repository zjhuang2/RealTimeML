...
uint16_t send_y[CAPTURE_SIZE + 2];
uint16_t send_z[CAPTURE_SIZE + 2];
...
void get_samples() {
  ...
  for (int i = 0; i < CAPTURE_SIZE; i++) {
    if (IMU.accelerationAvailable()) {
      ...
      send_x[i] = x_i;
      y_i = int(32768 * y / 1000 / 4 + 100); 
      send_y[i] = y_i;
      z_i = int(32768 * z / 1000 / 4 + 100); 
      send_z[i] = z_i;
    }
  }
}
...
void send_samples() {
  ...
  send_x[CAPTURE_SIZE + 1] = 0;

  // Channel number (channel 1 = 0, channel 2 = 1... channel n = n-1)
  send_y[CAPTURE_SIZE] = 1;
  // Frame completion byte (only after all channels sent)
  send_y[CAPTURE_SIZE + 1] = 0;

  // Channel number (channel 1 = 0, channel 2 = 1... channel n = n-1)
  send_z[CAPTURE_SIZE] = 2;
  // Frame completion byte (only after all channels sent)
  send_z[CAPTURE_SIZE + 1] = 1;
  ...
  // Send the delimiter
  Serial.write(delimiter, sizeof(delimiter));
  Serial.write((uint8_t *)send_y, sizeof(send_y));

  // Send the delimiter
  Serial.write(delimiter, sizeof(delimiter));
  Serial.write((uint8_t *)send_z, sizeof(send_z));
